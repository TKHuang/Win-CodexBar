...
Examples:
  bash scripts/gh-safe.sh --repo TKHuang/Win-CodexBar --verify-kind pr --target 361 --what-if -- pr comment 361 --body-file .review/comment.md
  bash scripts/gh-safe.sh --repo TKHuang/Win-CodexBar --verify-kind repo --what-if -- pr create --title "..." --body-file body.md
EOF
}
...
elif [[ "${repo,,}" != "tkhuang/win-codexbar" ]]; then
  echo "GitHub writes are not allowlisted for '$repo'. Expected TKHuang/Win-CodexBar." >&2
  exit 3
fi
